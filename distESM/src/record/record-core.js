...
export class RecordCore extends Emitter {
    constructor(name, services, options, recordServices, whenComplete) {
        ...
        this.stateMachine = new StateMachine(this.services.logger, {
            ...
            transitions: [
                { name: RA.SUBSCRIBE, from: 0 /* INITIAL */, to: 1 /* SUBSCRIBING */, handler: this.onSubscribing.bind(this) },
                ...
                { name: RA.READ_RESPONSE, from: 1 /* SUBSCRIBING */, to: 4 /* READY */, handler: this.onReady.bind(this) },
                ...
                { name: RA.DELETE, from: 4 /* READY */, to: 8 /* DELETING */ },
   ...
   */
    ...
    */
    ...
    handle(message) {
        ...
        if (message.action === RA.MESSAGE_DENIED ||
            message.action === RA.MESSAGE_PERMISSION_ERROR) {
            if (message.originalAction === RA.SUBSCRIBECREATEANDREAD ||
        ...
        if (message.action === RA.SUBSCRIPTION_HAS_PROVIDER ||
   ...
   */
    checkDestroyed(methodName) {
        if (this.stateMachine.inEndState) {
            this.services.logger.error({ topic: TOPIC.RECORD }, EVENT.RECORD_ALREADY_DESTROYED, { methodName });
            return true;
        }
        return false;
    }
