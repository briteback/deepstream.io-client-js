...
import { TOPIC, ACTIONS, META_KEYS, PAYLOAD_ENCODING, } from './message-constants';
import { ACTION_TO_WRITE_ACK, isWriteAck, } from './utils';
import { HEADER_LENGTH, META_PAYLOAD_OVERFLOW_LENGTH, } from './constants';
import { validateMeta, hasPayload, } from './message-validator';
export function getMessage(msg, isAck) {
    const message = msg;
...
function buildMultipart(topic, action, meta, payload) {
    ...
    let fin;
    do {
        const metaSectionLength = Math.min(metaLength - metaSectionOffset, META_PAYLOAD_OVERFLOW_LENGTH);
        const payloadSectionLength = Math.min(payloadLength - payloadSectionOffset, META_PAYLOAD_OVERFLOW_LENGTH);
